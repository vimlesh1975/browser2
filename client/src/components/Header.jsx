...
export const Header = () => {
    ...
    const backEndIP = process.env.REACT_APP_BACKENDHOST_IP;

    const dispatch = useDispatch();

    const logout = () => {
        localStorage.setItem('token', '{"token":""}');
        window.location.href = backEndIP + '/pbns/dmam/home/index.php';
    }
    const selectBg = e => {
    ...
    return (
        <>
            ...
            <nav onDoubleClick={showAndSelectColor} className="navbar navbar-expand-lg navbar-light" style={{ backgroundColor: bg }}>
                <div style={{ width: '100%' }}>
                    <button type="button" className="btn btn-warning btn-sm  mr-1"><a href={`${backEndIP}/pbns/dmam/home/index.php`}>HOME</a></button>
